...
\twocolpage{
  ...
  \begin{tabularx}{\linewidth}{@{}L{24mm}Y@{}}
    ...
    \midrule
    A0 → A1 & entrée dans le mode \texttt{AUTO}\\
  ...
  \begin{primarycard}
    ...
    Une unité de \texttt{nImp} correspond à une sortie suivie d'une rentrée. Le plateau ne passe au bac suivant qu'après quatre cycles complets.
  \end{primarycard}

  \begin{notebox}[colback=ekwarning,colframe=orange!35]{Référencement}
    \texttt{INIT} exploite le capteur inductif comme référence. La stratégie mécanique de recherche du zéro n'étant pas définie, la version réalisée considère \texttt{bZero = TRUE} comme condition de validation du référencement.
  \end{notebox}
}
