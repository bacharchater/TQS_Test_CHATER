...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node[anchor=north west,text=white!70,font=\fontsize{11}{14}\selectfont,
    ...
    {De la première logique\\de séquencement à une logique\\TwinCAT structurée et testée.};
  ...
  \node[anchor=south west,text=white!68,font=\small]
    at ([xshift=17mm,yshift=56mm]current page.south west)
  ...
  \node[anchor=north west,draw=ekline,fill=white,rounded corners=4mm,
    ...
    at ([xshift=112mm,yshift=-110mm]current page.north west) {%
      \textbf{Objet}\par
      \color{ekmuted}Présenter la version finale du carrousel, compilée, exécutée et validée sous TwinCAT XAE, avec son code source joint pour revue.\par\vspace{5mm}
      \color{ekink}\textbf{Périmètre}\par
      \color{ekmuted}Structured Text · huit bacs\\indexation pneumatique · réinitialisations et défauts\par\vspace{5mm}
      \color{ekink}\textbf{Entreprise}\par
      \color{ekmuted}TQS · Tri Qualité Service\par\vspace{5mm}
      \color{ekink}\textbf{Date}\par
      \color{ekmuted}4 septembre 2026
    };
  ...
  \node[anchor=south west,fill=eksoft,draw=ekprimary!25,rounded corners=4pt,
    ...
    {VERSION 2.0 · IMPLÉMENTATION ET ESSAIS};
\end{tikzpicture}
\null\newpage
