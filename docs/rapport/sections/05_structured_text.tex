...
\begin{minipage}[t]{0.605\textwidth}\vspace{0pt}
  \subsection{Commandes pneumatiques temporisées}
\begin{lstlisting}[style=stcode]
...
\end{lstlisting}
  ...
  \begin{notebox}{Commandes déterministes}
    Au début de chaque cycle PLC, les deux sorties sont placées à \texttt{FALSE}. Seul l'état actif commande ensuite \texttt{bVerinAv} ou \texttt{bVerinAr}. En l'absence d'entrées de fin de course, le \texttt{TON} simule la durée de chaque mouvement ; la fin de la phase arrière incrémente \texttt{nImp}.
  \end{notebox}
\end{minipage}
