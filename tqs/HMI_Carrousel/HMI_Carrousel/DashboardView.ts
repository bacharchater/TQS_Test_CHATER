...
export class DashboardView {
    ...
    private bacCounts: number[] | null = null;
    private fault: boolean | null = null;
    ...
    public setMode(modeValue: string | undefined): void {
    ...
    const valueElement =
        this.getElement('cycleStateValue');

    const hintElement =
        this.getElement('cycleStateHint');

    const rotationRing =
        this.getElement('rotationRing');
     ...
     */
    if (rotationRing) {
        rotationRing.classList.toggle(
            'tqs-carousel__ring-dash--moving',
            cycle === 'BAC_SUIVANT',
        );
    }
}
...
/* ============================================================
   ...
   ============================================================ */
    ...
    private updateBacPositions(): void {
    ...
    public setSetpoint(value: number | undefined): void {
    ...
    public setBacCounts(value: number[] | undefined): void {
    ...
    public setCylinderCycle(
        value: number | undefined,
    ): void {

        const text =
            this.getElement('cylinderCycleValue');

        const steps =
            this.getElement('cylinderSteps');
        ...
        if (value === undefined) {
            ...
            if (steps) {
                for (
                    let i = 0;
                    i < steps.children.length;
                    i++
                ) {
                    (
                        steps.children[i] as HTMLElement
                    ).className = '';
                }
            }
    ...
    public setFault(
        ...
        const led =
            this.getElement('faultLed');

        const text =
            this.getElement('faultValue');
...
private refreshBacs(): void {
    let totalPieces = 0;

    for (let i = 1; i <= this.nbBacs; i++) {
        const card =
            this.getElement('bin' + i + 'Card');

        const chip =
            this.getElement('bin' + i + 'Chip');

        const countElement =
            this.getElement('bin' + i + 'Count');

        const maxElement =
            this.getElement('bin' + i + 'Max');

        const barElement =
            this.getElement('bin' + i + 'Bar');
